...
export default function PaymentInScreen() {
    ...
    const { mutate: submitPayment, isPending } = useMutation({
        ...
        onSuccess: () => {
            ...
            Alert.alert('Payment In', 'Payment recorded successfully.', [
                { text: 'OK', onPress: () => router.back() },
            ]);
        },
        onError: (error) => {
            Alert.alert('Payment In failed', error instanceof Error ? error.message : 'Unable to record payment.');
        },
    });
    ...
    return (
        <SafeAreaView style={s.safe} edges={['top']}>
            <View style={s.header}>
                ...
                <Text style={s.title}>Payment In</Text>
                <View style={{ width: 44 }} />
            </View>
                ...
                <ScrollView contentContainerStyle={s.content}>
                    <View style={[s.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
                        ...
                        <Pressable
                            ...
                            disabled={isPending}
                        >
                            {isPending ? <ActivityIndicator color="#fff" /> : <Text style={s.submitText}>Save Payment In</Text>}
                        </Pressable>
                    </View>

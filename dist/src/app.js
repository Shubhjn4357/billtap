...
import { analyticsEvents, items, offers, orders, paymentIntents, phoneVerifications, plans, users } from './db/schema';
...
const getAuthUserFromRequest = async (authorizationHeader) => {
...
const ensurePlansSeeded = async () => {
    const countResult = await db.select({ count: sql `count(*)` }).from(plans);
    const count = Number(countResult[0]?.count ?? 0);
    if (count > 0) {
        return;
    }
    await db.insert(plans).values(DEFAULT_SERVER_PLANS.map((plan) => ({
        ...plan,
        createdAt: new Date(),
        updatedAt: new Date(),
    })));
...
const hasAudienceAccess = (audience, subscriptionStatus) => {
    if (audience === 'all')
        return true;
    if (audience === 'active_subscribers')
        return subscriptionStatus === 'active';
    if (audience === 'inactive_subscribers')
        return subscriptionStatus !== 'active';
    return false;
};
const isCronAuthorized = (provided, authorizationHeader) => {
...
app.get('/offers/active', optionalAuth, async (c) => {
    ...
    const data = await db
        .select()
        .from(offers)
        .where(and(eq(offers.isActive, true), sql `(${offers.startsAt} is null or ${offers.startsAt} <= ${now})`, sql `(${offers.endsAt} is null or ${offers.endsAt} >= ${now})`))
...
app.get('/items', requireAuth, async (c) => {
    ...
    if (queryText) {
        ...
        const data = await db
            .select()
            .from(items)
            .where(and(eq(items.userId, authUser.uid), or(sql `${items.nameLowercase} like ${`%${normalized}%`}`, sql `coalesce(${items.barcode}, '') like ${`%${queryText}%`}`)))
